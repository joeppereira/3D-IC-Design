...
*   [**Vertical Mesh**](reports/vertical_mesh.md): the surrogate's discretisation error turned out to be vertical rather than in-plane — measuring that first redirected the work, and z-refined labels take the total bias to **+0.08 K**.
*   [**Power Map Reality**](reports/power_map_reality.md): measured with OpenROAD on ASAP7 — real designs put **half their power in 15–19% of the area**, worth **+17…+21 °C** against the uniform blocks the search assumes.
*   [**Submodeling**](reports/submodel.md): the grid-convergence study converged the discretisation of a *smeared input* — rearranging a macro's watts inside its own footprint, at fixed total power, moves the peak **+36 °C**.
*   [**Thermal-Gradient Clock Skew**](reports/gradient_skew.md): the field we already solve, read as timing — **10.9–28.1 ps** of purely thermal skew across the feasible front, against the ~25 ps a 2 GHz clock tree has to spend.
